...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    @Query(value = "SELECT t.* FROM railway.product t " +
    ...
    @Query(value = "select p.* from railway.product p where slug = :slug", nativeQuery = true)
    ...
    @Query(value = "select p.* from railway.product p where status = 1", nativeQuery = true)
